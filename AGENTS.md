...
5. **SSR-First**: Ensure all components are SSR-safe. Do not use `window`, `document`, or client-side frameworks unless building a specialized "Nervous System" component.
...
- **Fail Fast**: If styling is missing, output a 🛑 CONSTRAINT TRIGGERED block for Layer 2.
...
3. **Type Synchronization**: Ensure that if a new variant is added to `@phcdevworks/spectre-ui`, the Astro component types update accordingly.
